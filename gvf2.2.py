...
The GVF learner predicts how long it will take until the motor starts moving
as it moves in a back and forth fashion with a variable gamma value using a
TD(lambda) algorithm.

Policy = back and forth position of motor
Cumulant = 1
Gamma = {0 if Motor Position = 1, 1 otherwise}
Alpha = 0.5
...
log_filename = "gvf_var_gamma.txt"
...
# ===========================================================================================================
time_step = 0
window_size = 50 # Larger to capture the full extent of motor movement
...
p.add_line(label="Goal Position")
...
while time_step <= 1000:
    ...
    gamma = 0 if abs(norm_goal_position - 1) < 0.01 else 1
    ...
    if len(cumulant_history) > window_size:
        ...
        true_return = 0
    norm_true_return = true_return/6
    ...
    norm_pred_pos = agent.predict/6
    
    # ===========================================================================================================
    # Logs the prediction, cumulant, and true return
    print(f"Time Step: {time_step}, Prediction:{norm_pred_pos:.2f}, Cumulant:{cumulant:.2f}, True Return:{norm_true_return:.2f}, Gamma: {agent.gamma}")
    ...
    p.update(time_step, [norm_pred_pos, norm_goal_position, norm_true_return])
    if time_step == 100 or time_step == 500 or time_step == 1000:
        p.save(f"var_gamma_step_{time_step}.png")
